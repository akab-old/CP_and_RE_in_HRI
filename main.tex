...
\documentclass[LaM,oneside,noexaminfo,english]{sapthesis}
...
% PDF Informations
\usepackage{hyperref}
\hypersetup{pdftitle={Integrating Conditional Planning and Robust Execution in Human-Robot Interaction},pdfauthor={Valerio Sanelli}}
...
%\def\gunther{\includegraphics[width=0.8\textwidth]{Figures/gunther.jpg}}

\title{Integrating Conditional Planning and\\ Robust Execution in Human-Robot Interaction}
\author{Valerio Sanelli}
\IDnumber{126714}

\course[Artificial Intelligence and Robotics]{Intelligenza Artificiale e Robotica\\
											(Master in Artificial Intelligence and Robotics)}
\courseorganizer{Facolt\`{a} di Ingegneria dell'informazione, Informatica e Statistica}
\AcademicYear{2015/2016}
\copyyear{2016}
\advisor{Prof. Luca Iocchi}
\authoremail{valerio.sanelli@gmail.com}
...
\dedication{Ai coraggiosi e ai tenaci.\\ Alla famiglia, all'amore e agli amici.}

\begin{abstract}
This thesis deals with the integration of conditional planning and robust execution in human-robot interaction.
In this work I have developed a framework for the generation of plans that are robust and reliable with respect to the context, i.e. social interaction between human and robots. The architecture proposed operates on plans with an additional layer of representation that adds the complexity necessary to produce robust plans.
With the proposed approach, we are able to provide plans that are more complex that the ones provided by any formalism/planner.
The work developed in this thesis was tested during the Maker Faire 2016 (Rome) event, where the proposed approach has proven to be flexible and robust. Furthermore, this work is part of an ongoing collaboration with the Imperial College of London to develop a framework for general and robust planning in ROS. We intend to submit the contribution produced in this thesis to the ICAPS 2017 conference. The framework is released as Open Source Software and available on-line.
\end{abstract}
...
\noindent The primary intent of this thesis is to present an architecture that produces robust conditional plans that may deals with the uncertainty present in the human-robot interaction context.

\noindent To motivate this work I will present in the next section \ref{sec:motivating} an example of a problem in HRI context.\\
In chapter \ref{sec:related_works} I will characterize the context of this work and the state of the art.\\ 
In section \ref{sec:plan_repr} I will review some of the formalisms in literature, showing their pros and cons in terms of representation expressivity and computation complexity.\\ 
In section \ref{sec:approach} I make some consideration about the analysis done in the precedent chapter and I will introduce the used approach.\\ 
...
\end{itemize}
I will focus on the aspects of the interaction and I do not consider other uncertainties coming from the perception system, since they are not relevant for this work.
...
\noindent The image shows that there is a person in front of shop 2 ($h2$), another wandering nearby shop 6 ($h3$) and a third person in front of him ($h1$). We will focus on $h2$. \\
\noindent The set of actions that the robot can perform is: \\ 
...
\noindent We assume that the robot is able, through its perception and planning systems, to acquire and represent knowledge about the environment and try to guess what people wishes.
...
\noindent Given these informations, the whole state space can be subdivided in two groups: one that describes generic information about the environment (robot and person location, the type of shop, etc.) and one for describing the knowledge of the robot about the human needs. In particular, we have that:\\
\centerline{$X = Y \cup Z$}
\noindent where $X$ represents the complete state space, $Y$ are the environmental variables while $Z$ are the variables that describe the human needs. \\
...
\begin{figure}[H]
	...
	\caption{A possible configuration for the mall problem.}
	\label{fig:mall_2}
\end{figure} 

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.45,trim=0cm 0cm 0cm 0cm]{images/coaches_graph.png}
	\caption{a sketch that show a possible solution plan for the problem of h2.}
	\label{fig:coaches_graph}
\end{figure}

\noindent Given the situation in figure \ref{fig:mall_2} we can reasonably assume that the person is interested in a clothes stores, but we cannot exclude others situations (for instance, he can be hungry). \\
\noindent A possible non-optimal plan that inspects $h2$ needs is depicted in \ref{fig:coaches_graph}.
\noindent We say non-optimal on purpose since the presented plan can infer only a property at the time. This is a plan that is guaranteed to terminate but only when multiple goals are defined (like in this case). If is it the case where only a goal is present, we need at least an edge going back to the initial place of the plan to guaranteed that the robot doesn't get stuck in some situation, and can traverse multiple paths to reach the goal. A more robust plan is shown in the following figure:
...
\noindent In the images, the squares ($ \square $) represents sensing actions.
A \textbf{sensing action} is an action that incorporates an observation (a boolean evaluation) and it is used by the robot to characterise its knowledge. We assume that a state in which at least one of the $Z$ properties holds is a goal state. As we can see from the example, in this kind of domains the initial state is partially known.

\noindent This example shows the needs for an infrastructure that is able to characterise the state of the world, taking into account all the possible outcomes that may arise when dealing with humans. We will resume this example in following chapters.
...
\newline
Human-robot Interaction studies the interaction between human and robots. It is a multidisciplinary field that attracts researcher from robotics, AI, cognitive psychology, and much more.
Its main objective is to develop models for natural and social interaction between humans and robots. 
A central idea in HRI research is that humans expect that robots exhibit a human-like behaviour when interacting with them, so they can feel comfortable as when interacting with a person. Is is important to underline that a social robot must interact with non-expert users, that don't know what to expect when dealing with it.\\
...
\textbf{Examples:} \textit{Project COACHES (\url{https://coaches.greyc.fr/}), Amelia the Assistant (\url{http://www.ipsoft.com/amelia/}), general assistance task}.\\
...
We will focus instead on the representation of the state space, relying on state of the art planners.

\section{Conditional Planning and Execution}\label{sec:approach}
...
For instance if we consider decidable logic formalisms, $ALCK_{NF}$ is one of the more expressive and yet the planning in general it's included in PSPACE, so too complex to run on a robot in realtime.\\ 
...
\noindent This brief analysis shows that there is a compromise between expressivity and complexity. 
Our work is developed in a middle situation, in which the space of the state is not entirely known but the unknown properties represent only a small observable set.
...
If we denote as $Z$ the states that represents the human-needs and with $Y$ the generic states, we have that:
\begin{center}
%$|Y|$ + $|2^Z| \ll |2^X|$ and $X = Y \cup Z$
$X = Y \cup Z$
\end{center}
If we maintain the number of unknowns $Z$ small, the problem becomes tractable. In fact, the number of sensing actions will be $|Z|$, and we have $1+2^{|Z|}$ possible initial states.
...
\end{itemize}
\noindent As we can see, PDDL (ROSPlan version), KPDDL, ASP and RDDL seems to fit our prerequisites. For this work we decided to consider PDDL version 2.1 plus two of his extensions, namely KPDDL and PDDL for CLG. \\
\noindent Once we have identified the languages that matches our prerequisites, we use them to represent and solve tests domains. Once a plan is computed, our architecture translate it into a conditional plan as we will discuss later.\\
\newline
\noindent Our aim is to define an architecture capable of translate different formalisms results into a conditional plan that will be refined and enhanced using the PNP formalism, that is much more expressive that any other formalism. \\
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.4, trim=75mm 0mm 0mm 30mm]{images/system_architecture_v2.png}
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.5,trim=2cm 0cm 0cm 0cm]{images/generator.png}
...
Notice that the translations of the planners output are mutually exclusive and do not run in parallel, that is we operate only on one planner output at the time.
...
The subset of languages/planners chosen in this thesis are PDDL, KPDDL and CLG PDDL.\\
...
PDDL (vers. 2.1) doesn't have syntax constructs that allow conditioning on action effects and this lead to the born of different extension, like KPDDL.
PDDL planners usually generate total ordered plans.
...
\end{center}
$t_n$ are terms that represents atomic actions (\textit{a}) or conditional branches, where (\textit{c}) are the conditions. \\
\textit{c} could represents a boolean sensing, like in the definition above, or an action sensing with multiple outcomes:
\begin{center}
...
a3; a4
\end{verbatim}
This represent a plan with two conditional branches at the beginning and an additional branching in the second of the two branches.	
...
The planners induce the following semantical structures in the solution:
\begin{itemize}
...
The algorithm responsible for the generation of the PNP performs a graph expansion using the conditional plan representation.\\
...
In CLG the description of the problem P, which represent a non-deterministic search problem in belief space, is translated into a non-deterministic problem X(P) in state space.\\
...
Anyway, without an informed heuristic, the planner run time grows exponentially in the number of the properties to sense.
...
In this section I will show for each introduced formalisms the correspondent PNP generated. The examples are the ones presented in the previous section.
%\newline
%\newline
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.6, trim=5cm 0cm 0cm -5cm]{images/hanoi_3.png}
	\caption{PNP generated from the example in \ref{sec:seq}. This is a solution for the hanoi problem with three disks.}
	\label{fig:inline_example}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{PNP generated from the example in \ref{sec:dag}. The problem represents a medical diagnosis. Notice the plan structure that represents a direct graph.}
	\label{fig:inline_example}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{PNP generated from the the example in \ref{sec:digraph_transl}. The problem is a file search in a Unix system. Notice the plan structure that represents a tree.}
	\label{fig:inline_example}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{PNP generated from the example in \ref{sec:diy}.}
	\label{fig:inline_example}
\end{figure}

\chapter{Use Cases}\label{sec:use_cases}
In this section I will characterize some examples, describing the whole process from the problem definition to the plan generation implemented by our architecture.\\ In order to do so I will start from the motivating example introduced in section \ref{sec:motivating}.
...
\end{verbatim}
Beside from predicates that model environmental properties, we notice the ones that describe agents state. 
\noindent For what concern actions, I will not list all of them but I rather focus on the ones that represent deterministic sensing actions:
\begin{verbatim}
(:action sense-person
                      :parameters ( ?r ?p)
...
\end{verbatim}
\noindent In this scenario, we can imagine the classical infrastructure with a planning system responsible of the generation of the problem instances and a module responsible for gathering the informations through sensors and physical interfaces and that manages fluents. This is to justify the presence of "\textit{desire-fashion h2}" ground in the initial state description.\\
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.48, angle=270, trim=0mm 0mm 0mm 0mm]{images/coaches_pddl.png}
...
\noindent KPDDL allows to represent conditional plans. The cons reside in the fact that depending on the written domain, the planner could take much time to compute a solution. The author provides the possibility of driving the solution search by the means of a heuristic factor that varies in the range of $[0.0 - 1.0]$, indicating with 0 a breadth-first search while with 1 a depth-first search.\\ 
In any case, this not represents a safe solution as the algorithm is not complete (see \ref{sec:analysis}).
...
\end{verbatim}
\noindent We do not need to model explicitly in the initial state the knowledge about the person needs.\\
...
\noindent We translate the solution in our conditional plan representation format. Notice that the KPlanner output doesn't differ so much from our representation, this is meant to be the same for every considered formalisms (present and future).
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.5, angle=270, trim=0mm 0mm 0mm 50mm]{images/coaches_kpddl.png}
...
In this example we have progressively augmented the human-needs variables to see how the planner performs.
...
It is clear that if we double again the unknowns, the planner runs indefinitely.

\newpage
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.5, angle=270, trim=0mm 0mm 0mm -80mm]{images/kpddl_ex2.png}
...
This planner is included in the ROSPlan framework.
CLG has a very effective heuristic that can present instantaneously a solution, when present, or communicate that this not exist. 
The ROSPlan framework is integrated into ROS and, due to its modularity, it can be extended to consider other formalisms and planners.\\
...
The integration between ROSPlan and our infrastructure is an ongoing work and I will discuss possible future implementations in the next chapter.

\subsection{Example 1: COACHES v.3}
This is a simplified version of the coaches example, in which the only unknowns are in the person position.
...
This is the complete coaches example, as presented in \ref{sec:motivating}. Namely, we try to infer the person needs using the following sensing action:
\begin{verbatim}
  (:action ask
...
\noindent We do not show the complete PNP since it is too big to fit in a page. Consider that for this case there are 211 places and 422 edges. As we can see from the image \ref{fig:rosplan_ex2}, the generated PNP is complete in the sense that it discern between all different cases (\textit{food}, \textit{clothes}, \textit{obj} etc.) and the generation happens instantaneously. 
\noindent This example concludes our analysis.


\begin{figure}[H]
	\centering
	\includegraphics[scale=0.5,angle=270, trim=0mm 0mm 0mm 0mm]{images/rosplan_coaches2.png}
...
\section{Discussion}
As we have seen in the examples, we have found a set of satisfying formalisms to describe and represents conditional plans. I will now revise the cons and pros of all approaches.\\ 
Planning in PDDL using FF reveals to be very fast. The planner relies on the fact that many of classical problems present a similar structure.
PDDL 2.1 lack of the expressivity necessary to describe sensing actions, but the planner algorithm is complete and so reliable for simple applications.
KPDDL has the syntactic power that matches our prerequisites and it was easy to describe our reference domain. However, the provided planner (KPlanner) is not scalable. It is in fact shown that if we increase the number of the unknown variables in the planning domains, the running time for the algorithms increase exponentially. 
The CLG approach reveals to be the most promising since it unites the advantage of the PDDL approach with the expressivity of KPDDL. Its reliability resides in the fact that was build upon FF, so it shares with it all his benefits. The only compromise is that we must express the range of values that the observed properties should take. A small price to pay, considered that is hypothesis is suitable in most cases and that its algorithm is complete.
...
The contribution produced in this thesis is part of an ongoing work with the ROSPlan authors. We are actively collaborating to extend further this approach. Our aim is to provide an infrastructure for general and robust planning under ROS.
The obtained results will be submitted at the ICAPS conference 2017.\\
Finally, the software developed in this thesis is released under Open Source License.
...
\noindent Talking about formalisms to be added, we should certainly consider probabilistic formalisms.  In fact, it could be useful to take advantage of plans that represent uncertainty also quantitatively. Not only, given the modular nature of the infrastructure, the procedure to add a new translator module is very simple.\\
