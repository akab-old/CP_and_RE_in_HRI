...
Writing domains and waiting for a planner to compute a solution is a long processes and is not always needed.
For instance, think about the case in which one wants simply to test the integration of a new perception module with the rest of the robotic architecture.
So, as additional feature, we provide the possibility to directly explicit the conditional plan.
A plan of this kind can be written in a txt file and has the following form:\\
\begin{center}
 $ P = t_1; t_2; ... ; t_n$\\
 $t_n = a \,\, | <c \,\, ? \,\, t_1; ... ; t_{n-1} : (not \,\, c) \,\, ? \,\, t_n; ...; t_m >$\\
\end{center}
where $t_n$ are terms that represents atomic actions (\textbf{a}) or conditional branches over conditions (\textbf{c}). \\
\textbf{c} could be a boolean evaluation, like in the definition above, or a multi branch choice:
\begin{center}
...
\end{verbatim}
This gives raise to the following conditional plan in fig. \ref{fig:inline_example}.
The correspondent algorithm is showed in .
%\newpage
\begin{figure}[H]
	\centering
