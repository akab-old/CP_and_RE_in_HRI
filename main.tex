...
initialize $\pi_{cp}$\;
\ForEach{pair in $v_p$}{
	state\_name = first of pair\;
	state\_action = second of pair\;
...
In order to achieve our results, it was necessary to extend PNPgen framework with modules that allow to automatically generate PNPs from the discussed planners outputs.\\
Summarizing, the languages considered for the generation, along with planners and the semantic structures are:
\begin{itemize}
...
\noindent In the following I will discuss the algorithm that is responsible for PNPs generation.
