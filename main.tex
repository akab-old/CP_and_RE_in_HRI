...
\end{verbatim}\\
\noindent As this certainly increase the robustness of the computed plans, at the same time the search space increase considerably. Consider For instance an state space of observable properties of dimension $N$. If we consider only boolean evaluations, the complete space increases as $O(2^N)$. So we must be very careful when dealing with this kind of representation. The discussion of search algorithms that can exploit, for instance, the structure of the problem are beyond the discussion of this thesis. Instead, I will talk about the translation of this kind of plans into an executable PNP.
...
\subsection{Sequential Translator}
...
This is also our case since the chosen planner (Fast-Forward) provides us with an ordered list of actions. 
%It estimates a heuristic by relaxing the problem and apply to it a GRAPHPLAN algorithm.
This kind of solution is interpreted as linear plans. 
...
\eIf{$\sigma = 0$}{
	...
	$\lambda = CondPlan\_addSerialAction(\sigma)$\;
	\Return $\sigma + seq\_transl(\pi')$\;}
}
\end{algorithm}}\\
...
\noindent The algorithm reads actions from the FF output; $CondPlan\_addSerialAction()$ creates a conditional plan state and links it to one created earlier. \\
%\newline
...
%Furthermore, the generation of this kind of plans is fast and the planner algorithm complete.

\newpage
\subsection{DAG Translator}
...
\Fn{$DAG\_transl(\pi)$ : $\lambda$}{	
	...
	\eIf{$\sigma$ = $0$}{
		...
		\eIf{$\sigma =a_i$}{
			$\lambda$ = $CondPlan\_addSerialAction(a)$\;		
		}{
		\If{$\sigma = \phi ? a_1 ; a_2$}{
			$\lambda_1$ = $DAG\_transl(a_1)$\;
			$\lambda_2$ = $DAG\_transl(a_2)$\;
			$\lambda$ = $CondPlan\_addSensing(\phi,\lambda_1,\lambda_2)$\;
		}
		}
	\Return $DAG\_transl(\pi')$\;	
	}
...
\begin{figure}[H]
	...
	\caption{PNP generated from KPlanner output. The problem represents a medical diagnosis. Notice the plan structure that represents a direct graph.}
...
The solution is semantically interpreted as a tree.
...
The algorithm is similar to the ones of KPDDL; the previous is, in fact, a particular case in which only two observations are involved. In the general case, multiple observation may occur. Not only, a branch could contain other branches, making the recursion definition of plans possible.\\
%\scalebox{0.8}{
%\begin{algorithm}[H]
...
\SetKwProg{Fn}{Function}{}{}
\Input{$\pi$: the plan}
\Output{$\lambda$: the PNP corresponding to $\pi$}
\Fn{$tree\_transl(\pi)$ : $\lambda$}{
	...
	{
		\eIf{$\sigma$ = $a_i$}{
			$\lambda$ = $CondPlan\_addSerialAction(a_i)$\;
		}
		{
			\If{$\sigma = \phi_1 ? \lambda_1 : ... : \phi_n ? \lambda_n$}{
				$\Phi$ = $CondPlan\_splitBranch(\sigma)$\;
				\ForEach{$\phi$ in $\Phi$}{
					$\lambda_{1..n}$ = $tree\_transl(\phi)$\;
				}
				$\lambda$ = $CondPlan\_merge(\lambda_{1..n})$\;	
			}
		}
	}
...
%Due to the completeness of the planner algorithm, is it possible to use this approach on-line on a ROS node. 
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.41,trim=5cm 0cm 0cm 5cm]{images/unix.png}
	\caption{PNP generated from CLG output. The problem represents a file search in a Unix system. Notice the plan structure that represents a tree.}
	\label{fig:inline_example}
\end{figure}

\subsubsection{DIY Conditional Plan}
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.6,angle=270,trim=5cm 0cm 0cm 0mm]{images/inline_ex3.png}
...
\noindent In order to be sure that every node is visited only once, and thus to avoid dangerous loops in the generation procedure, we maintain a stack of the visited nodes.
...
The algorithm is showed in \ref{cond_gen}.
...
\section{Discussion}
As we have seen in the examples, we have found a possible set of satisfying formalisms to describe and represents conditional plans.
I will now revise the cons and pros of all approaches. Planning in PDDL using FF reveals to be very fast.
FF uses a relaxed GRAPHPLAN algorithms with an informed search based on goal distance. The derived search algorithm is called enforced hill-climbing. Beside from the fact that this version of the language lack of the expressivity necessary to describe epistemic states, this kind of approach rely on the fact that many problems have a similar structure. Namely, they present a search space. Nonetheless, the algorithm is complete.
KPDDL has the syntactic power that matches our prerequisites. It was easy to describe our reference domain. However, the provided planner (KPlanner) is not complete.
The CLG approach reveals to be the most promising since it unites the advantage of the PDDL approach with the expressivity of KPDDL. Its reliability resides in the fact that was build upon FF, so it shares with it all his benefits. The only compromise is that we must express the range of values and observed property could take. A small price to pay, considered that is hypothesis is suitable in most cases and that its algorithm is complete.
...
Another aspect to take into account regards the execution rules described in \ref{sec:ER}. These rules are easy to define and enhance the robustness of our approach. One could consider implementing a module that generates automatically, from a common sense knowledge base like \cite{liu2004conceptnet}, the execution rules needed. \\
...
A long-term development could be to expand the architecture with a tool that merges or concatenates the plans obtained from different formalisms. This would allow taking advantage of the benefits of each approach while overcoming the computation problems that was discussed in \ref{sec:analysis}. 
