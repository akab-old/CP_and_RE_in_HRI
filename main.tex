...
\begin{verbatim}
    digraph plan_#N{
        0[label= "sense_person_shop4" ];
        1[ label= "move_shop4_shop5_r" ];
                     ...
        M[ label= "m-th action"];
        "0" -> "1" [ label="A" ];
        "0" -> "20" [ label="(not(A))" ];
...
\Input{$\pi$: a Conditional Plan,\\
 	   $s_f$: $\pi$ final state\\
 	   $M_{SA}$: A Map between state-action -> outcomes}
...
\Output{$\pi_c$: PNP generated from a Conditional Plan}
 %\KwResult{how to write algorithm with \LaTeX2e }
  $curr\_state \leftarrow \pi.first$\; 
  $\pi_c \leftarrow addCondition(initial)$\;
  $M_V \leftarrow insert(curr\_state)$\;
  $SS \leftarrow push(curr_state)$\;
 \While{$(SS \neq 0)$}{
