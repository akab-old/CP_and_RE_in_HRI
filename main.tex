...
\paragraph{Social Rules} Norms of human being imitated/reproduced by the robot in order to encourage collaboration and that are socially acceptable.\\ 

\noindent
Cooperation doesn't mean necessarily social, and this is the case of classic automation approach. The most promimenent cases are the ones in which the human is encouraged to collaborate by the means of a reward (griedy case). Human are also curious, so encourage this part could also help in establish and mantain a collaboration through time. (curiosity case)
The third and most interesting case for this study is the scenario in which the robot itself starts the collaboration. There are many aspects to take into account in this, and I will enlist the most important in the following.
...
Main Questions:
\begin{enumerate}
\item How to start/mantain interaction
\item How to represent parallel/serial interaction
\item How to represent/update common knowledge
\end{enumerate}
...
In this cases the interaction between the human and the robot is minimal. The human is a target and the interaction is a secondary objective. No representation of the human state is requred.
\begin{itemize}
\item Patrolling
\item Search \& Rescue
\end{itemize}
...
Those cases represents the most significant for this work. In this case the interaction is the principal objective and the robot itself could start the interaction. A representation of the human state is mantained.
\begin{itemize}
\item COACHES
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.5, angle=270, trim=20mm 10mm 10mm 10mm]{images/go_and_follow_person.png}
	\label{fig:mall_1}
\end{figure}
...
\subsubsection{PNPgen}
PNPgen\cite{pnp_gen} is a library included the Petri Net Plans distribution responsible for the automatic generation of PNPs from a formal definition provided.\\
...
\item Once produced,the PNP can be extended of modified to deal with unexpected outcomes that may rise during the execution of an action and not considered in the domain, either manually or automatically as we will discuss later.
\end{enumerate} 


The languages considered are two:
\begin{itemize}
\item PDDL (FF planner): this produce a linear plan.
\item KPDDL (kplanner): this produce a conditional plan, or more generally a DAG (direct acyclic graph).
\item ROSPlan (modified kplanner): similar to KPDDL, produce a tree. 
\end{itemize}
