...
The concatenation operator $+$ in this case has a more general meaning, since this algorithm works with both DAG and trees.\\ 
In fact in case of trees, we have that for the leaves the algorithm will return $\lambda + \{\}$.
For the general case of DAGs, the algorithm
...
For the generation, we read these states and generate the correspondent PNP.\\
the Conditional Planning approach guarantees the robustness of the computed plan, though the search space increase considerably. 
For a state space of observable properties of dimension $N$, if we consider only boolean evaluations, the space grows as $O(2^N)$. 
In this case however, we decouple the two types of variables (epistemic and environmental) and we rely on the completeness of the chosen planners.
This guarantees a safe execution of the framework on a robot.\\
\noindent We decide to consider also PDDL in this work for multiple reasons.\\ 
...
\noindent In the following section I will show the correspondent PNPs for the example presented in the Translator Modules section \ref{sec:transl_modules}. 
In the next chapter I will show some examples and results to demonstrate the approach. \\
